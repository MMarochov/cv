// skills.js

export const skills = [ // List of contact information
    {
        name: "Geospatial Data Analysis",
        text: "QGIS, ArcGIS, SQL, PostGIS, Remote Sensing Imagery Processing (Aerial, Sentinel-2, Landsat), GeoPandas, MovingPandas, Shapely, Rasterio, osdatahub"
    },
    {
        name: "Data Science",
        text: "Python, Flask, Pandas, Matplotlib, Numpy, Scikit-learn, Scikit-image"
    }, 
    {
        name: "Platforms, Software and Agile Methodologies",
        text: "Azure Machine Learning Studio, Databricks, Anaconda, Mamba, Git, GitHub, Azure DevOps, Scrum developer, Scrum master"
    },
    {
        name: "Web Development",
        text: "HTML, CSS, JavaScript, Leaflet, Svelte, Hugo, Node, NPM"
    },
    {
        name: "Prototyping",
        text: "Creative Problem Solving, Human-Centered Design, Collaborative Working"
    }, 
    {
        name: "Soft Skills",
        text: "Writing, Customer Relationships, Presenting Complex Ideas to Diverse Audiences, Research, Consultancy, Technical Mentorship and Knowledge Transfer, Curious Learner, Pair and Mob Programming, Adaptability, Active Listening, Proactive and Positive Attitude"
    }
];